...
using Rant.Internals.Engine.Compiler.Syntax.Richard.Operators;
using Rant.Internals.Engine.Utilities;
using Rant.Internals.Stringes;

namespace Rant.Internals.Engine.Compiler
{
	internal class RichardCompiler
	{
		internal enum RichardCompilerMode
		{
			Expression,
			/// <summary>
		...
		public RichardCompiler(string sourceName, string source, TokenReader reader, RantCompiler parentCompiler)
		{
		...
		public RantAction Read(RichardCompilerMode type = RichardCompilerMode.Expression)
		{
			List<RichActionBase> actions = new List<RichActionBase>();

			Token<R> token = null;
			if (type == RichardCompilerMode.Expression)
				_hitEndOfExpr = false;
			...
			while (!_reader.End)
			{
				...
				switch (token.ID)
				{
					...
					case R.Whitespace:
						if (type == RichardCompilerMode.InvertValue)
							goto done;
						break;
					case R.LeftSquare:
						{
							if (!actions.Any())
							{
								...
								actions.Add((Read(RichardCompilerMode.List) as RAExpression).Group);
								break;
							}
							var val = Read(RichardCompilerMode.BracketValue) as RAExpression;
						...
						break;
					case R.RightSquare:
						if (type == RichardCompilerMode.List)
							goto done;
						if (
							type == RichardCompilerMode.FunctionBody ||
							type == RichardCompilerMode.VariableValue)
							_hitEndOfExpr = true;
						if (
							type != RichardCompilerMode.Expression &&
							type != RichardCompilerMode.VariableValue &&
							type != RichardCompilerMode.BracketValue &&
							type != RichardCompilerMode.FunctionBody)
							SyntaxError(token, "Unexpected end of expression.");
						goto done;
					case R.LeftParen:
						{
							var group = (Read(RichardCompilerMode.ExpressionGroup) as RAExpression).Group;
							...
							if (_reader.PeekLooseToken().ID == R.Equal)
							{
								...
								var body = Read(RichardCompilerMode.FunctionBody);
								actions.Add(new RichFunction(token, (body as RAExpression).Group, group as RichGroup));
								if (type == RichardCompilerMode.VariableValue)
									goto done;
								if (_reader.PrevLooseToken.ID == R.RightParen && type == RichardCompilerMode.ExpressionGroup)
									goto done;
								break;
							}
					...
					case R.RightParen:
						if (
							type == RichardCompilerMode.ExpressionGroup ||
							type == RichardCompilerMode.VariableValue ||
							type == RichardCompilerMode.ExpressionGroupNoList ||
							type == RichardCompilerMode.FunctionBody)
							goto done;
						break;
					case R.LeftCurly:
						...
						if (_reader.PeekLooseToken().ID == R.ConstantLiteral || _reader.PeekLooseToken().ID == R.Text)
						{
							...
							if (_reader.PeekLooseToken().ID == R.Colon)
							{
								...
								var obj = Read(RichardCompilerMode.KeyValueObject);
						...
						if (type == RichardCompilerMode.FunctionBody || type == RichardCompilerMode.Expression)
						{
							var body = Read(RichardCompilerMode.ExpressionBlock);
							actions.Add(body as RichActionBase);
							if (type == RichardCompilerMode.FunctionBody)
								goto done;
							break;
						}
						Unexpected(token);
						break;
					case R.RightCurly:
						if (type == RichardCompilerMode.ExpressionBlock)
						{
							var group = new RichGroup(actions, token, _sourceName);
							return new RichBlock(token, new List<RichActionBase>() { group });
						}
						if (type == RichardCompilerMode.KeyValueObjectValue)
							return new RichGroup(actions, token, _sourceName);
						Unexpected(token);
						break;
					case R.Semicolon:
						if (type != RichardCompilerMode.Expression && type != RichardCompilerMode.ExpressionBlock)
							goto done;
						if (actions.Count == 0)
							break;
					...
					case R.ConstantLiteral:
						if (type == RichardCompilerMode.KeyValueObject)
						{
							var name = token;
							_reader.ReadLoose(R.Colon, "key value seperator");
							var value = Read(RichardCompilerMode.KeyValueObjectValue) as RichGroup;
						...
						if (_keywords.IndexOf(token.Value) >= 0)
						{
							switch (token.Value)
							{
								case "var":
									...
									{
										_reader.ReadToken();
										var val = Read(RichardCompilerMode.VariableValue) as RAExpression;
								...
								case "function":
									{
										...
										var argGroup = Read(RichardCompilerMode.ExpressionGroup) as RAExpression;
								...
								case "if":
									{
										if (type != RichardCompilerMode.Expression && type != RichardCompilerMode.ExpressionBlock && type != RichardCompilerMode.FunctionBody)
											Unexpected(token);
										_reader.ReadLoose(R.LeftParen, "if statement param");
										var val = Read(RichardCompilerMode.ExpressionGroupNoList) as RAExpression;
										var body = Read(RichardCompilerMode.FunctionBody) as RAExpression;
										RichActionBase elseBody = null;
										if (!_reader.End && _reader.PeekLooseToken().Value == "else")
										{
											_reader.ReadLooseToken();
											elseBody = (Read(RichardCompilerMode.FunctionBody) as RAExpression).Group;
										}
								...
								case "return":
									{
										if (type != RichardCompilerMode.ExpressionBlock && type != RichardCompilerMode.FunctionBody && type != RichardCompilerMode.Expression)
											Unexpected(token);
										RichActionBase expr = null;
										if (_reader.PeekLooseToken().ID != R.Semicolon)
											expr = (Read(RichardCompilerMode.VariableValue) as RAExpression).Group;
										actions.Add(new RichReturn(token, expr));
										if (_reader.PrevLooseToken.ID == R.Semicolon && type == RichardCompilerMode.FunctionBody)
								...
								case "while":
									{
										...
										var test = (Read(RichardCompilerMode.ExpressionGroupNoList) as RAExpression).Group;
										var body = (Read(RichardCompilerMode.FunctionBody) as RAExpression).Group;
										actions.Add(new RichWhile(token, test, body));
									}
									break;
								case "break":
									{
										if (type != RichardCompilerMode.FunctionBody && type != RichardCompilerMode.ExpressionBlock)
											Unexpected(token);
								...
								case "for":
									{
										...
										var expr = (Read(RichardCompilerMode.ExpressionGroupNoList) as RAExpression).Group;
										var body = (Read(RichardCompilerMode.FunctionBody) as RAExpression).Group;
					...
					case R.Comma:
						if (type == RichardCompilerMode.KeyValueObjectValue)
							return new RichGroup(actions, token, _sourceName);
						if (type == RichardCompilerMode.ExpressionGroup || type == RichardCompilerMode.VariableValue || type == RichardCompilerMode.List || type == RichardCompilerMode.Expression)
						{
					...
					case R.Exclamation:
						...
						if (!_reader.End && (_reader.PeekLooseToken().ID == R.LeftParen || _reader.PeekLooseToken().ID == R.Text))
						{
							...
							{
								_reader.ReadLooseToken();
								rightSide = (Read(RichardCompilerMode.ExpressionGroup) as RAExpression).Group;
								actions.Add(new RichPrefixInvert(token) { RightSide = rightSide });
								break;
							}
							rightSide = (Read(RichardCompilerMode.InvertValue) as RAExpression).Group;
							actions.Add(new RichPrefixInvert(token) { RightSide = rightSide });
							break;
						}
						Unexpected(token);
					...
					case R.Equal:
						...
						if (actions.Last() is RichVariable)
						{
							...
							var val = (Read(RichardCompilerMode.VariableValue) as RAExpression).Group;
							actions.RemoveAt(actions.Count - 1);
						...
						if (actions.Last() is RichObjectProperty)
						{
							...
							var value = Read(RichardCompilerMode.VariableValue) as RAExpression;
			...
			if ((actions.Any(x => x is RichArgumentSeperator) &&
				(type == RichardCompilerMode.Expression ||
				type == RichardCompilerMode.VariableValue) || type == RichardCompilerMode.List))
			{
				// lists
			...
			else if (actions.Where(x => x is RichArgumentSeperator).Any() && type != RichardCompilerMode.ExpressionGroup)
				Unexpected(actions.Where(x => x is RichArgumentSeperator).First().Range);

			switch (type)
			{
				case RichardCompilerMode.ExpressionGroup:
				case RichardCompilerMode.ExpressionGroupNoList:
				case RichardCompilerMode.VariableValue:
				case RichardCompilerMode.Expression:
				case RichardCompilerMode.BracketValue:
				case RichardCompilerMode.FunctionBody:
				case RichardCompilerMode.List:
				case RichardCompilerMode.InvertValue:
					return new RAExpression(actions, token, _sourceName);
				default:
		...
		private RichList CreateListFromActions(RichardCompilerMode type, List<RichActionBase> actions)
		{
			...
			listActions.Add(lastItem);
			return new RichList(actions.First().Range, listActions, type != RichardCompilerMode.List);
		}
