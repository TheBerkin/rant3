...
namespace Rant.Core.Stringes
{
	...
	internal sealed class Lexer<T> : IEnumerable where T : struct
	{
		...
		/// </summary>
		public HashSet<T> IgnoreRules { get; }

		internal _<Func<Stringe, Stringe>, T> UndefinedCaptureRule { get; private set; }
		internal _<string, T> EndToken { get; private set; }
		...
		{
			foreach (var t in types) IgnoreRules.Add(t);
			return this;
		}

		/// <summary>
		/// Returns the symbol that represents the specified identifier. If the identifier cannot be found, the method will return
		/// an empty string.
		/// </summary>
		...
		public void AddEndToken(T endTokenId)
		{
			...
			EndToken = _.Create("EOF", endTokenId);
		}
		...
		public void AddEndToken(T endTokenId, string endTokenValue)
		{
			...
			if (_sorted) throw new InvalidOperationException("Cannot add more rules after they have been used.");
			EndToken = _.Create(endTokenValue, endTokenId);
		}
		...
		{
			if (_sorted) throw new InvalidOperationException("Cannot add more rules after they have been used.");
			UndefinedCaptureRule = _.Create(evalFunc, tokenId);
		}

		/// <summary>
		/// Adds a constant rule to the context. This will throw an InvalidOperationException if called after the context is used
		/// to create tokens.
		/// </summary>
		...
		public void Add(string symbol, T value, SymbolPriority priority = SymbolPriority.Last)
		{
			...
			(priority == SymbolPriority.First ? _listHigh : _listNormal).Add(_.Create(symbol, value,
				StringComparison.InvariantCulture));
			_punctuation.Add(symbol[0]);
		}

		/// <summary>
		/// Adds a constant rule to the context that affects all symbols in the specified array. This will throw an
		/// InvalidOperationException if called after the context is used to create tokens.
		/// </summary>
		...
		public void Add(string[] symbols, T value, SymbolPriority priority = SymbolPriority.Last)
		{
			...
			foreach (string s in symbols)
			{
				...
				(priority == SymbolPriority.First ? _listHigh : _listNormal).Add(_.Create(s, value,
					StringComparison.InvariantCulture));
