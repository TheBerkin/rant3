...
namespace Rant.Core.Constructs
{
	...
	{
		[RantDescription("A random element is selected each time.")] None,
		[RantDescription("Executes from left to right.")] Forward,
		[RantDescription("Executes from right to left.")] Reverse,
		...
		//ReverseSkip,
		[RantDescription("Shuffles items and executes in order. Re-shuffled each time all items are used up.")] Deck,
		[RantDescription("Shuffles items and executes in order. The same order is reused for each traversal.")] Cdeck,
		[RantDescription("Chosen randomly, the same element is selected each time.")] Locked,
