...
namespace Processus
{
    internal class SourceReader
    {
        ...
        {
            if (End) throw new ProcessusException(_source, null, "Unexpected end of file.");
            return _tokens[_pos++];
        }
        ...
        public bool Take(TokenType type, bool allowEof = true)
        {
            ...
            {
                if (!allowEof)
                    throw new ProcessusException(_source, null, "Unexpected end-of-file.");
                return false;
            }
        ...
        {
            if (End) 
                throw new ProcessusException(_source, null, "Expected " + (expectedTokenName ?? "'" + Lexer.Rules.GetSymbolForId(type) + "'") + ", but hit end of file.");
            if (_tokens[_pos].Identifier != type)
            {
                throw new ProcessusException(_source, _tokens[_pos], "Expected " + (expectedTokenName ?? "'" + Lexer.Rules.GetSymbolForId(type) + "'"));
            }
            return _tokens[_pos++];
        }
        ...
        public IEnumerable<IEnumerable<Token<TokenType>>> ReadMultiItemScope(TokenType open, TokenType close, TokenType separator, BracketPairs bracketPairs)
        {
            ...
            while (!End)
            {
                ...
                else if (bracketPairs.ContainsClosing(token.Identifier) || close == token.Identifier) // Previous bracket allows nesting
                {
                    ...
                    {
                        throw new ProcessusException(_source, token, 
                            "Invalid closure '"
                            + lastOpening.Value
                            + " ... " 
                            + token.Value 
            ...
            throw new ProcessusException(_source, null, "Unexpected end of file - expected '" + Lexer.Rules.GetSymbolForId(close) + "'.");
        }
