...
namespace Processus.Blueprints
{
    internal sealed class TagBlueprint : Blueprint
    {
            ...
            : base(interpreter)
        {
            ...
            {
                throw new ProcessusException(Source, Name, "A function of the name '" + Name.Value + "' does not exist.");
            }

            if ((_tagDef = defs.GetDef((argData == null ? 0 : argData.Length))) == null)
            {
                throw new ProcessusException(Source, Name, "The function '" + Name.Value + "' does not contain a signature that accomodates " + (argData == null ? 0 : argData.Length) + " argument(s).");
            }
