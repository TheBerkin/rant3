...
using System.Linq;
using Processus.Arithmetic.Parselets;
...
namespace Processus.Arithmetic
{
    internal partial class Parser
    {
        ...
        public Expression ParseExpression(int precedence = 0)
        {
            ...
            {
                throw new ProcessusException(_src, token, "Invalid expression '" + token.Value + "'.");
            }
            ...
            while (GetPrecedence() > precedence)
            {
                ...
                {
                    throw new ProcessusException(_src, token, "Invalid operator '" + token.Value + "'.");
                }
        ...
        public Token<MathTokenType> Take(MathTokenType type)
        {
            ...
            {
                throw new ProcessusException(_src, token, String.Concat("Expression expected ", type, ", but found ", token.Identifier, "."));
            }
            return token;
        }
